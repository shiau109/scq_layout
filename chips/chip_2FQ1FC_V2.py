...
class Chip2FQ1FCV2(ASlib):
    ...
    def _produce_xyline(self, launcher_tag, qubit_name, factor_x, factor_y, factor=1):
        self.insert_cell(
            WaveguideComposite,
            nodes=[Node(self.refpoints["launcher_"+launcher_tag+"_base"]),
                   ...
                   Node(pya.DPoint(self.refpoints["launcher_"+launcher_tag+"_base"].x + factor_x*1000 - factor*abs(factor_y)*1000,
                                   self.refpoints[qubit_name+"_port_xyline"].y)),
